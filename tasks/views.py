...
def index(request):
    ...
    for t in TodoItem.objects.all():
        if t.get_priority_display() not in priority:
            priority[t.get_priority_display()] = 0
        priority[t.get_priority_display()] += 1
...
def get_time(request):
    ...
    #     date_time = datetime.datetime.now().strftime("%H.%M.%S %Y-%m-%d")
    #     cache.set('time', date_time, 300)
    date_time = datetime.datetime.now().strftime("Время: %H.%M.%S Дата: %Y-%m-%d")
    return render(request, "tasks/time.html", {
                                                # 'time': cache.get('time')
                                                'time': date_time
                                            })
...
class TaskListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["tags"] = dict(pairs=zip(tags, user_tasks))
        return context
